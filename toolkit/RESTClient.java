...
public class RESTClient {
    ...
    public RESTClient(String hostName, String username, String password){
        ...
        SSLContext context;
        try {
            //Test code, currently running in Java 7. So TLSv1.2 will enable TLSv1, TLSv1.1, TLSv1.2
...
//        service.addFilter(new LoggingFilter(System.out));
    ...
    /**
     * Returns a String response for the resource.
     * @param path request URL
     * @return
     * @throws ApiException
     */
    public String getList(String path) throws ApiException{
        try {
            WebResource r = service.path(baseUrl + path);
            return r.accept(MediaType.APPLICATION_XML).get(String.class);
        } catch(UniformInterfaceException e){
            e.printStackTrace();
            handleErrors(e.getResponse());
        }
    ...
    /**
     * Make a post request to API with json payload.
     *
     * @param payload the JSON string payload
     * @param path path to the service
     * @return service response
     */
    public <T extends BaseApiBean> T createAndGet_JSONPayload(String payload, String path, Class beanClass) throws ApiException{
        try {
            ClientResponse response = service.path(baseUrl + path).
            ...
            return get((Class<T>)beanClass, get(response));
        } catch(UniformInterfaceException e){
            handleErrors(e.getResponse());
        }
    ...
    /**
     * Make a post request with empty payload
     * @param path request URL
     */
    public <T extends BaseApiBean> ClientResponse create(String path) throws ApiException{
        try{
            ClientResponse response = service.path(baseUrl + path).
                    post(ClientResponse.class, new byte[]{});
    ...
    private String get(ClientResponse response) {
    ...
    public void updateEmptyBody(String path, boolean async) throws ApiException {
        try {
            ClientResponse response = service.path(baseUrl + path).queryParam("async",String.valueOf(async))
                    .header("Content-Length", "0").type("text/plain").put(ClientResponse.class, new byte[]{});
