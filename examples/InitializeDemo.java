...
public class InitializeDemo {
    ...
    public static final String CVP_A_USERNAME = "cvp.a.username";
    public static final String CVP_A_PASSWORD = "cvp.a.password";
    public static final String CVP_B_USERNAME = "cvp.b.username";
    public static final String CVP_B_PASSWORD = "cvp.b.password";
    ...
    private void updateCvp() {
        ...
        List<MachineHost> machines = typeToMachine.get(MachineType.CVP);
        MachineHost cvpA = findMachineBySide(MachineType.CVP, "sideA");
        MachineHost cvpB = findMachineBySide(MachineType.CVP, "sideB");
        MachineService svc = new MachineService();
        svc.setType(ServiceType.CVP_WSM);

        if(cvpA != null) {
            MachineAddress address = findAddressByType(cvpA.getAddresses(), AddressType.PUBLIC);
            svc.setUserName(props.getProperty(CVP_A_USERNAME));
            svc.setPassword(props.getProperty(CVP_A_PASSWORD));
            address.setMachineServices(Arrays.asList(svc));

            restClient.update(cvpA);
        }
        if(cvpB != null) {
            MachineAddress address = findAddressByType(cvpB.getAddresses(), AddressType.PUBLIC);
            svc.setUserName(props.getProperty(CVP_B_USERNAME));
            svc.setPassword(props.getProperty(CVP_B_PASSWORD));
            address.setMachineServices(Arrays.asList(svc));
            restClient.update(cvpB);
        }
    }

    /**
     * Updates CVP reproting
     */
    private void updateCvpReporting() {
        System.out.println("Updating CVP Reporting");

        MachineHost cvpReportingB = findMachineBySide(MachineType.CVP_REPORTING, "sideB");
        MachineService svc = new MachineService();
        svc.setType(ServiceType.DIAGNOSTIC_PORTAL);

        if(cvpReportingB != null) {
            MachineAddress address = findAddressByType(cvpReportingB.getAddresses(), AddressType.PUBLIC);
            svc.setUserName(props.getProperty(CVP_A_USERNAME));
            svc.setPassword(props.getProperty(CVP_A_PASSWORD));
            address.setMachineServices(Arrays.asList(svc));

            restClient.update(cvpReportingB);
        }
    }
    ...
    private void updateCm() {
        ...
        MachineHost machine;
        machine = findMachineByName(MachineType.CM, cmPubName);
    ...
    private void updateDeploymentTypeInfo() {
        ...
        deploymentTypeInfo.setHardwareLayoutType(HardwareLayoutType.TRC);
    ...
    private MachineAddress findAddressByType(List<MachineAddress> addresses, final AddressType type) {
        return Iterables.find(addresses, new Predicate<MachineAddress>() {
            ...
            @Override
            public boolean test(MachineAddress addr) {
                return addr.getType().equals(type);
            }
        });
    }
    ...
    private MachineHost findMachineByName(MachineType type, final String name) {
        ...
        try {
            return Iterables.find(machines, new Predicate<MachineHost>() {
                ...
                @Override
                public boolean test(MachineHost machine) {
                    return machine.getName().equals(name);
                }
            });
    ...
    private Iterable<InitializationStatus> findStatus(List<InitializationStatus> statuses, final StateEnum state) {
        try {
            return Iterables.filter(statuses, new Predicate<InitializationStatus>() {
                ...
                @Override
                public boolean test(InitializationStatus initializationStatus) {
                    return initializationStatus.getState() == state;
                }
            });
        } catch (NoSuchElementException ignore) {
